            ...
            if (访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                ...
                if (url.searchParams.get('token') === 订阅TOKEN) {
                    ...
                    if (订阅类型 === 'mixed') {
						...
						// 1. 读取环境变量
						const JP_SOCKS = env.SOCKS5_JP || ''; 
						const SG_SOCKS = env.SOCKS5_SG || ''; 

						// 2. 核心逻辑：定义最终路径 (Override 模式)
						let 最终路径 = "";
						const 备注小写 = 节点备注.toLowerCase();
						const 随机前缀 = config_JSON.随机路径 ? 随机路径() : "";

						// 判断逻辑：
						// 如果命中特定国家，使用 /socks5= 前缀 + 对应国家的账号
						// 注意：这里改回了 /socks5= ，这意味着：
						//      - 只有 GO2SOCKS5 变量里的白名单域名，才会走这个日本/新加坡 VPS
						//      - 其他域名的流量，依然走 Cloudflare Workers 直连
						if ((备注小写.includes('东京') || 备注小写.includes('日本') || 备注小写.includes('jp') || 备注小写.includes('japan')) && JP_SOCKS) {
							最终路径 = 随机前缀 + `/socks5=${JP_SOCKS}`; 
						} 
						else if ((备注小写.includes('新加坡') || 备注小写.includes('狮城') || 备注小写.includes('sg') || 备注小写.includes('singapore')) && SG_SOCKS) {
							最终路径 = 随机前缀 + `/socks5=${SG_SOCKS}`;
						} 
						else {
							// 没命中特定国家，保持默认配置 (通常也是 /socks5=默认账号 或 /proxyip=...)
							最终路径 = 随机前缀 + 节点路径;
						}

						// 3. 生成链接
						const 节点HOST = 随机替换通配符(host);
						return `${协议类型}://${config_JSON.UUID}@${节点地址}:${节点端口}?security=tls&type=${config_JSON.传输协议}&host=${节点HOST}&sni=${节点HOST}&path=${encodeURIComponent(最终路径) + TLS分片参数}&encryption=none${config_JSON.跳过证书验证 ? '&allowInsecure=1' : ''}#${encodeURIComponent(节点备注)}`;
						// === 修改结束 ===								
...
// === 1. 新增：检测域名是否在白名单中 ===
    let 匹配白名单 = false;
    if (SOCKS5白名单 && Array.isArray(SOCKS5白名单)) {
        const lowerHost = host.toLowerCase();
        for (const item of SOCKS5白名单) {
            const domain = item.toLowerCase();
            // 支持 *通配符，例如 *google.com 匹配 www.google.com
            if (domain.startsWith('*')) {
                if (lowerHost.includes(domain.slice(1))) { 匹配白名单 = true; break; }
            } else {
                // 精确匹配或子域名匹配
                if (lowerHost === domain || lowerHost.endsWith('.' + domain)) { 匹配白名单 = true; break; }
            }
        }
    }
    // ===================================
    ...
    // === 2. 修改后的执行逻辑 ===
    // 逻辑：如果 SOCKS5 开启，且 (开启了全局模式 OR 命中了白名单)，则强制走 SOCKS5
    if (启用SOCKS5反代 && (启用SOCKS5全局反代 || 匹配白名单)) {
        try {
            await connecttoPry();
        } catch (err) {
            throw err;
